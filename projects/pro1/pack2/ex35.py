...
try:
    ...
    listData = ['마우스', '키보드']
    tuplData = (dictData, listData)

    with open('hello.dat', mode='wb') as fobj3:  # 객체를 저장할때는 .dat, .obj 확장자
    # mode ='wb' : 바이너리로 저장
        pickle.dump(tuplData, fobj3)    # pickle.dump(대상, 파일객체)
        pickle.dump(listData, fobj3)    # list 타입 객체만 저장
    print('특정 객체를 파일로 저장')
