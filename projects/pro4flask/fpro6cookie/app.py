...
# render_template_string : 문자열로 작성한 jinja 템플릿을 렌더링 해 HTML로 변환
# redirect : 다른 url로 이동
# url_for : 라우트 함수 이름으로 URL을 안전하게 생성하는 함수

app = Flask(__name__);

# cookie는 브라우저에 저장되는 작은 키-값 데이터이고, 서버가 클라이언트와 연결 상태를 유지 하는 것처럼 할 수 있다. 
...
def set_cookie():
    ...
    # 먼저 "read_cookie 페이지로 이동하라"는 redirect 요청을 만들고 그 응답에 따라 쿠키를 추가한 뒤 브라우저에 돌려줌
    resp = make_response(redirect(url_for("read_cookie")))
    resp.set_cookie(    # 브라우저에 쿠키 저장
        key="name",     # 쿠키 이름
        value=name,     # 사용자가 입력한 쿠키 값
        max_age=60 * 5,   # 유효시간 - 5분 뒤 만료. 기본값은 세션 기간동안(보통 1년)
        httponly=True,    # Javascript에서 document.cookie로 접근 불가(보안상 이유)
...
def delete_cookie():
    # 쿠키 삭제 후 홈(/)으로 이동하기 위한 redirect 응답 생성
