...
# 파이썬 세션(session)은 웹에서 사용자 ㅈ어보를 서버에 저장하는 기능을 말함(쿠키를 통해 세션 운영)
# 일정 시간동안 동일 사용자(브라우저)와 일련의 요청을 하나의 상태로 보고 그 상태를 유지시키는 기술
# 쿠키에 비해 상대적으로 안전함

# 실습 : 사용자가 os를 선택하면 세션에 저장하고 읽기
from datetime import timedelta # 날짜/시간을 가감해서 기간 설정 하기에 용이

app = Flask(__name__);

# Flask는 세션 사용을 위해 secret_key 설정이 필요
app.secret_key = "abcdef123456"     # 위조 방지용 비밀키값
# 참고 : 키값 자동 생성 => 터미널창 -> python -c "import secrets; print(secrets.token_hex)"

app.permanent_session_lifetime = timedelta(seconds=5)   # 세션 만료 시간 5초 설정 (기본 30분)
