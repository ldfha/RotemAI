...
df = pd.read_csv('ex1.csv')     # csv 파일 읽어서 dataframe으로 생성
print(df, type(df)) 
print()
df = pd.read_table('ex1.csv', sep=',')  # 구분자 필요. 없으면 열 전체를 하나로 읽음
df = pd.read_table('ex1.csv', sep=',', skip_blank_lines=True)  # 구분자 필요. 없으면 열 전체를 하나로 읽음
# skip_blank_line : 칼럼명, 데이터의 앞 공백을 제거
print(df)
print()
...
for i, chunk in enumerate(pd.read_csv(csv_fname, chunksize=chunk_size)):
    ...
    print(f"Chunk {i+1}번째 학생 : ID={first_student['id']}, 이름={first_student['name']}",
...
print(f"score1 총합 : {total_score1}, score1 평균 : {average_chunk1:3f}")
print(f"score2 총합 : {total_score2}, score2 평균 : {average_chunk2:3f}")
print('전체 한 번에 처리 시간 :', round(time_all, 7))
print('청크로 처리한 총 시간 :', round(time_chunk_total, 7))
