...
sql = """create table if not exists extab(
        product varchar(10), maker varchar(10), weight real, price integer)"""

conn = sqlite3.connect(":memory:")  # 실행하는 동안에만 존재
conn.execute(sql)
conn.commit()

data=[('mouse','samsung',12.5,5000), ('keyboard','LG',52.5,32000)]
