...
# ax1.hist(np.random.randn(1000), bins=10, alpha=0.9)  # bins : 데이타 집계 구간, alpha : 투명도
# ax2.plot(np.random.rand(10))     # 기본 선 그래프
# plt.show()

# # 막대그래프
# data = [50, 80, 100, 90, 70]
# plt.bar(range(len(data)), data)     # 세로 막대
# plt.show()

# err = np.random.rand(len(data))
# plt.barh(range(len(data)), data, alpha=0.4, xerr=err)     # 가로 막대
# plt.show()

# # 원
# plt.pie(x=data, colors=['yellow', 'blue', 'red'], explode=[0, 0.2, 0, 0.1, 0])# 파이조각분리
# # 데이터는 시계 반대방향으로 돈다
# plt.title('원 그래프')
# plt.show()
