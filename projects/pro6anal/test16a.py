...
# 정규성, 등분산성
x1 = np.array(data[data.ta_gubun == 0].AMT.astype(float).to_numpy())     # 추움 그룹 매출액
x2 = np.array(data[data.ta_gubun == 1].AMT.astype(float).to_numpy())     # 보통
x3 = np.array(data[data.ta_gubun == 2].AMT.astype(float).to_numpy())     # 더움
print(x1[:5])

print()
print(stats.levene(x1, x2, x3).pvalue)  # 0.039002  만족X
print(stats.bartlett(x1, x2, x3).pvalue)    # 0.018684
print()
print(stats.shapiro(x1).pvalue)     # 0.24819 만족 O
...
# statistic=99.190801202, pvalue=2.3607371010895207e-34
# 해석 : pvalue < alpha 0.05이므로 귀무 기각
# 어느 음식점의 매출 데이터는 온도에 따라 매출액 평균에 차이가 있다.

print()
# 정규성 깨지면 stats.kruskal() 사용, 등분산성이 깨지면 welch's ANOVA 사용
# stats.kruskal()
print(stats.kruskal(group1, group2, group3))    # 귀무 기각
# KruskalResult : statistic=132.7022591, pvalue=1.527814e-29
print()
# welch's ANOVA
