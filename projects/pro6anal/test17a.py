...
# 귀무 : 교호작용이 있다. (태아수와 관측자 수는 관련이 있다)
...
print(result)
#                     sum_sq    df            F        PR(>F)
# C(태아수)          324.008889   2.0  2113.101449  1.051039e-27    : 귀무기각
# C(관측자수)           1.198611   3.0     5.211353  6.497055e-03
# C(태아수):C(관측자수)    0.562222   6.0     1.222222  3.295509e-01
# Residual          1.840000  24.0          NaN           NaN

# 태아수 PR(>F) 1.051039e-27 < 0.05 : 귀무기각. 태아 수와 태아의 머리둘레 평균은 차이가 있다.(유의미하다.)
# 관측자수 PR(>F) 6.497055e-03 (0.006470) < 0.05 : 귀무기각. 관측자수와 태아의 머리둘레 평균은 차이가 있다.(유의미하다)
# 상호작용 PR(>F) 3.295509e-01 (0.3) > 0.05 : 귀무 채택. 태아수와 관측자수는 관련이 없다.
# 해석 : 
...
# 귀무 : 교호작용이 있다. (poison 종류와 treat(응급처치) 방법은 관련이 있다)
...
# 요인별 레벨의 표본수는 4로 동일 (모든 집단별 표본수 동일하므로 균형설계가 잘 됨)
...
# 독 종류 PR(>F) 3.331440e-07 < 0.05 : 귀무기각. poison 종류와 독 퍼짐 시간의 평균은 차이가 있다.(유의미하다.)
# 처치방법 PR(>F) 3.777331e-06 < 0.05 : 귀무기각. treat(응급처치) 방법과 독 퍼짐 시간의 평균은 차이가 있다.(유의미하다)
# 상호작용 PR(>F) 1.122506e-01 (0.1) > 0.05 : 귀무 채택. poison 종류와 treat(응급처치) 방법은 관련이 없다.

# 사후분석
from statsmodels.stats.multicomp import pairwise_tukeyhsd
tukResult1 = pairwise_tukeyhsd(endog=data2.time, groups=data2.poison)
print(tukResult1)
tukResult1.plot_simultaneous(xlabel='mean of time', ylabel='poison')
plt.show()
plt.close()

tukResult2 = pairwise_tukeyhsd(endog=data2.time, groups=data2.treat)
print(tukResult2)
tukResult2.plot_simultaneous(xlabel='mean of time', ylabel='treat')
plt.show()
plt.close()