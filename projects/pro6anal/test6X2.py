...
print('chi2 : ', chi2)  # 6.544667820529891
print('p : ', p)    # 0.5864574
print('dof : ', dof)    # 자유도 : 8
print('expected : ', expected)  # 예측된 기대도수

# 해석 : 유의수준 0.05 < p:0.5864574 이므로 귀무 채택. 우연히 발생한 자료라고 할 수 있다.
...
# 귀무 : 연령대별로 sns 이용률 현황은 동일하다
# 대립 : 연령대별로 sns 이용률 현황은 동일하지 않다
...
ctab2 = pd.crosstab(columns=data['service'], index=data['age'])
print(ctab2)
chi2, p, dof, expected = stats.chi2_contingency(ctab2)
print('chi2 : ', chi2)  # 102.752024
print('p : ', p)    # 1.1679064204212775e-18
print('dof : ', dof)    # 자유도 : 8
print('expected : ', expected)  # 예측된 기대도수
...
samp_data = data.sample(n=500, replace=True, random_state=1).reset_index(drop=True)
print(samp_data.head(), ' ', len(samp_data))

ctab3 = pd.crosstab(columns=samp_data['service'], index=samp_data['age'])
print(ctab3)
chi2, p, dof, expected = stats.chi2_contingency(ctab3)
print('chi2 : ', chi2)  # 36.2074843
print('p : ', p)    # 1.6088950709449814e-05
print('dof : ', dof)    # 자유도 : 8
print('expected : ', expected)  # 예측된 기대도수