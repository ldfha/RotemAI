...
print(len(data))    # 30행이 넘으면 중심 극한 정리에 의해 정규성을 따른다고 가정
# 30개가 넘지 않으므로 정규성 검정 실시
# Shapiro-Wilk 검정은 가설검정의 방법으로 데이터가 정규분포를 가지는 지에 대해 검정하는 방법
# p값은 alpha 0.05보다 커야 정규성을 따른다고 할 수 있다.
print(stats.shapiro(data['국어']))  # pvalue 0.01295975
# alpth 0.05 > pvalue 0.01295975    정규성을 만족하지 않음
...
# WilcoxonResult(statistic=74.0, pvalue=0.39777620658
# alpha 0.05 < pvalue 0.397776 이므로 귀무가설 채택

print()
result = stats.ttest_1samp(data['국어'], popmean=80)
print("result : ", result)
# TtestResult(statistic=-1.33218016, pvalue=0.19856051, df=19
...
print(fdata, ' ', len(fdata))   # 18명
print('여아 몸무게 평균 :', np.mean(fdata.weight))      # 3132.44444
# 2800과 3132는 평균에 차이가 있는가?
print('여아 몸무게 표준편차 :', np.std(fdata['weight']))    # 613.78789

# One-sample t-test
result2 = stats.ttest_1samp(fdata['weight'], popmean=2800)
print("result2 :", result2)
# result2 : statistic=2.233187, pvalue=0.0392684, df=17
# 해석1 (p값) : alpha 0.05 > pvalue=0.03926 이므로 귀무가설 기각
# 해석2 (t분포표) : t값 2.233187, df 17, alpha 0.05, cv(임계값)? 1.740
#                t값이 cv 값 오른쪽(귀무 기각 영역)에 있으므로 귀무가설 기각

print('~~~~~~~~~~~~~~~~~')
# 선행조건인 정규성 검정을 한 경우
print(stats.shapiro(fdata['weight']))   # pvalue 0.0179847
# alpha 0.05 > pvalue 0.0179    정규성을 만족하지 않음

# 정규성 만족여부 시각화
sns.histplot(fdata['weight'], kde=True)     # 왜곡된 데이터 분포를 확인
plt.show()

# 정규성 만족여부 시각화 1 Quantile-Quantile plot
...
print(result3)
# WilcoxonResult statistic=37.0, pvalue=0.0342330
# 해석3 (p값) : alpha 0.05 > pvalue=0.0342 이므로 귀무가설 기각