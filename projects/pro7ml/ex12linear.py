...
# 독립변수 x를 정규화하기 (0 ~ 1 사이의 범위 내 자료료 변환)
scaler = MinMaxScaler()
x_scaled = scaler.fit_transform(x.reshape(-1, 1))   # 2차원 입력
print(x[:5])
print(x_scaled[:5])
...
print('절편(bias, intercept) :', model.intercept_)
...
def myRegScoreFunc(y_true, y_pred):
    ...
    # 오차를 제곱해 평균 구함(오차가 커질수록 손실함수 값이 빠르게 증가함. 값이 작으면 모델 성능이 ㅜㅜ)
    ...
    print(f"mean_squared_error(MSE, 평균제곱오차) : {np.sqrt(imsi)}")   # 9.281592 
...
print(x2[:5])
print(y2[:5])
print('상관계수 :', np.corrcoef(x2, y2)[0, 1])  # -0.05484441740006399

x_scaled2 = scaler.fit_transform(x2.reshape(-1, 1))   # 2차원 입력
model2 = LinearRegression().fit(x_scaled2, y2)
print('model :', model2)
print('회귀계수(slope) :', model2.coef_)
print('절편(bias, intercept) :', model2.intercept_)
print('결정계수(R^2) :', model2.score(x_scaled2, y2))   # 0.003007910119952384
# 분산이 너무 달느 데이터로 만든 모델은 의미 없음.
