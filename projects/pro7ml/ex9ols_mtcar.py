...
# Prob (F-statistic):1.79e-07 유의함  R-squared: 0.602 60%
# 절편 :  30.0989, 기울기 : -0.0682
# yhat = -0.0682 * x + 30.0989 + err
print('마력수 110에 대한 연비 예측값 :', -0.0682 * 110 + 30.0989)                              # 22.5969
print('마력수 110에 대한 연비 예측값 :', result.predict(pd.DataFrame({'hp':[110]})).values)    # 22.59375

print('\n다중선형회귀 ------')
result2 = smf.ols(formula='mpg ~ hp + wt', data=mtcars).fit()
print(result2.summary())
# 37.2273  -0.0318  -3.8778
print('마력수 100 + 무게 5에 대한 연비 예측값 :', (-0.0318 * 110) + (-3.8778 * 5) + 37.2273)                    # 14.3403
print('마력수 100 + 무게 5에 대한 연비 예측값 :', result2.predict(pd.DataFrame({'hp':[110], 'wt':[5]})).values) # 14.34309224
...
print(f'차체무게 {mtcars.wt.iloc[0]}일 때 예상 연비는 {new_pred.iloc[0]}')
# 차체무게 6.0일 때 예상 연비는 5.218296731005985