...
# LogisticRegression : 다중 클랙스(label, 종속변수)를 지원하도록 일반화 됨
...
print(iris.target)
print(iris.data[:3])
print(np.corrcoef(iris.data[:, 2], iris.data[:, 3])[0, 1])  # 0.9628654314027963

x = iris.data[:, [2, 3]]
y = iris.target
print(x.shape, ' ', y.shape)    # (150, 2)   (150,)
print(x[:3], y[:3], set(map(int, y)))   # {0, 1, 2}
...
# print(ori_x_train[:3])
# # 하지만 iris dataset은 크기의 차이가 거의 없으므로 표준화는 의미 없음

print('분류 모델 생성 ---------')
# model = LogisticRegression(random_state=0)
# multi_class 파라미터는 pip install scikit-learn==1.4.2 <-- 다운그레이드 필요. python 3.14에선 지원 X
# model = LogisticRegression(C=0.1, solver='lbfgs', multi_class='multinomial', random_state=0)

# 최신 버전은 multi_class 명시하지 않아도 똑같이 동작함
model = LogisticRegression(C=100.0, solver='lbfgs', random_state=0)
# 모델에 패널티 적용1 - C : L2 규제. 숫자값을 조정해가며 정확도 확인
# 최적화 알고리즘 - solver='lbfgs', 
...
# 만약 표준화된 자료로 모델을 생성했다면 new_data도 표준화해야 함
# sc.fit(new_data)
...
print(read_model.predict_proba(new_data))   # softmaz의 출력값
