# 과적합(Overfitting) 방지 목적 : train-test split, K-Fold, GridSearchCV
...
print('예측값 :', pred2)
print('실제값 :', y_test)
print('분류 정확도 :', accuracy_score(y_test, pred2))   # 0.95  효과 : 과적합 여부 확인

print('\n과적합 방지 목적의 처리 2 - 교차검증(cross validation)')
# train data를 k개만큼 분할해 학습과 평가를 병행하는 방법 - K-Fold가 가장 일반적
...
cv_acc = []
print('iris data :', features.shape)    # 150 by 4
# KFold 학습시 전체 150행이 학습 데이터(4/5, 120개), 검증데이터(1/5, 30개)로 분할되어 학습

n_iter = 0
# KFold 객체의 split()을 호출하면 Fold 별 학습용, 검증용 데이터의 행 인덱스를 array로 변환
...
print('cv_acc :', cv_acc)
print('평균 검증 정확도 :', np.mean(cv_acc))    #  0.8933320
...
# 연습용으로 일부 파라미터만 사용
# max_depth : 노드의 최대 깊이
# min_samples_split : 노드 분할을 위한 최소한의 샘플 수로 과적합 제어
...
# best_score_, best_params_, best_estimaotr_, grid_score_, ...
