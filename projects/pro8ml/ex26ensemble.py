...
# 강력한한 하나의 모델 보다는 약한 모델 여러개를 조합하여 더 정확한 예측 결과를 도출
import pandas as pd
...
# stratify=y : train, test 비율 유지 (불균형 데이터 모델 평가 시 왜곡 방지)
y_li = y.tolist()
...
print('test 분포 :', Counter(yts_li))     # Counter({1: 72, 0: 42})
...
print('roc auc score :', roc_auc_score(y_test, voting.predict_proba(x_test)[:, 1]))    # 0.994047619047619