...
# 산단테르 은행의 고객만족 여부 분류 처리
# https://www.kaggle.com/datasets/ioramishvili/santaner?select=train.csv
# 클래스(label)명은 target, 0:만족, 1:불만족
...
unsatistied_cnt = df[df['TARGET'] == 1].TARGET.count()
total_cnt = df.TARGET.count()
print(f'불만족 비율은 {unsatistied_cnt/total_cnt}') # 0.03956

# print(df.describe())    # feature의 분포 확인, v3 이상치 발견
...
y_label = df.iloc[:, -1]
print('x_featrues shape :', x_features.shape)   # (76020, 369)

# train / test split
x_train, x_test, y_train, y_test = train_test_split(x_features, y_label, test_size=0.3, random_state=12)
...
print('예측값 :', pred[:5])
print('실제값 :', x_test[:5].values)

from sklearn import metrics
print('분류 정확도 :', metrics.accuracy_score(y_test, pred))    # 0.9618521
...
print('gridcv의 최적의 파라미터 :', gridcv.best_params_)
...
print(f'xgb_roc_score : {xgb_roc_score:.5f}')   # 0.80556
# gridcv 최적의 파라미터 : {'colsample_bytree': 0.5, 'max_depth': 5, 'min_child_weight': 1}
...
print(f'xgb_roc_score2 : {xgb_roc_score:.5f}')   # 0.83591
...
from sklearn import metrics
print('분류 정확도 :', metrics.accuracy_score(y_test, pred2))    # 0.9618521
