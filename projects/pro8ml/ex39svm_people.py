...
# whiten : 주성분의 스케일이 작아지도록 조정
x_low = m_pca.fit_transform(faces.data)     # (1348, 2914) -> (1348, 10)
print('x_low :', x_low, ' ', x_low.shape)   # (1348, n_components)
...
plt.suptitle('Eigen Faces(주성분 얼굴)', fontsize=12)
plt.tight_layout()
plt.show()      # 출력 이미지는 실제 얼굴이 아니라 특징 패턴(얼굴 윤곽, 눈위치, 코그림자 ...)
# SVM 알고리즘은 실제 얼굴이 아니라 특징 패턴으로 분류작업을 한다.

print('설명력 확인')
print(m_pca.explained_variance_ratio_[:10])
print('누적 설명력')
print(m_pca.explained_variance_ratio_.sum())    # 0.9039585
# n = 100개로 얼마나 원본 정보를 유지했는지 확인
...
plt.tight_layout()
plt.show()  # 원본과 복원된 이미지의 기본 특징은 크게 차이가 없다.
...
print('예측값 :', pred[:5])     # [3 3 3 3 3]
print('실제값 :', y_test[:5])   # [3 5 4 2 4]
...
#  [  0   0   1   3   0   0  11   0]
#  [  0   0   0  12   0   0   0  24]]
print('accuracy :', accuracy_score(y_test, pred))       # 0.77448
...
#     George W Bush       0.65      1.00      0.79       133
...
#      weighted avg       0.85      0.77      0.76       337
...
# PCA 누적 분산 그래프 (왜 n_components=n)
import numpy as np
...
# 현재 모델의 분류 accuracy : 0.77448
# 실습 1 : 기존 데이터로 테스트
test_img = faces.data[0].reshape(1, -1)     # (1, 2914) 형태로 변환 - 모델이 이 형태로 학습함
print('test_img :', test_img)   #  [[0.53464055 0.5254902  0.49673203 ...
...
# 실습 1 예측결과 : Colin Powell, index : 1
# 실제값 : Colin Powell, indexl : 1

print()
# 실습 2 : 새로운 데이터로 분류하기
# 단계 : 이미지 읽기 -> 흑백변환(컬러일 경우) -> 크기 맞추기(62 x 47) -> 1차원으로 변환 -> 예측
...
# 이미지는 라이브러리마다 축 순서가 다름

img_np = np.array(img)  # numpy 변환
# print('img_np :', img_np)
# test_img : [0.53464055 0.5254902  0.49673203 
# img_np : [[52 51 50 ... 72 74 75] [51 50 49 ... 72 73 74] ... 0 ~ 255 숫자로 구성
# img_np를 test_img처럼 정규화 해야함
...
print('실습 2 예측 결과 :', faces.target_names[new_pred[0]], ', index :', new_pred[0])
