...
# SkinThickness : 피부의 두께, Insulin : 인슐린 수치, BMI : 체질량 지수
# DiabetesPedigreeFunction : 당뇨 혈통 함수, Age : 나이, Diabetes : 당뇨 유무
x = data.data
y = data.target
print(x[:2])
print(y[:2])
...
# 각 모델을 순서대로 반복 처리 : best model 추출, 성능 저장
...
plt.title(f'Best Model : {best_modelname}')
plt.xlabel('실제값')
plt.ylabel('예측값')
plt.show()