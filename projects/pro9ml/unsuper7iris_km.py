# KMeans : iris dataset - 군집분석, 정량평가, 클러스터별 평균비교
import numpy as np
import pandas as pd
...
# adjusted_rand_score : 군집과 실제 라벨 비교
...
print(x_scaled[:2])     # [[-9.00681170e-01  1.01900435e+00 ...

# KMeans 모델
k = 3
kmeans = KMeans(
    n_clusters=k, init='k-means++', n_init=10, random_state=42
    # n_init=10 : 10회 수행하며 가장 오차(inertia)가 작은 결과를 선택
)
...
# row_0            
# 0       0  50   0     ⇽ setosa
# 1      39   0  11     ⇽ versicolor (?) 섞임 - 거리기반으로 분류했기 때문
...
for col in feature_names:   # 각 군집별 데이터 분리
    ...
    print(f'{col} : f-statistic={f_stat}, p-value={p_val:.4f}')
...
# KMeans가 꽃받침, 꽃잎 길이/너비를 제대로 군집 분석했음을 알 수 있다.
