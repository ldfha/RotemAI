...
# KMeans 결과 시각화
plotResult(x, pred1)    # 설정에 따라 무조건 2개로 분류, 반달을 기준으로 잘림

print('DBSCAN으로 군집 분류')
db = DBSCAN(eps=0.2, min_samples=5, metric='euclidean')
# eps=0.2 - 샘플간 최대 거리, min_samples=5 - 점에 대한 이웃 샘플수(반경 포인트)
pred2 = db.fit_predict(x)
print('DBSCAN 군집 id :', pred2[:10])
print('군집 종류 :', set(pred2))    # 0, 1  => 이상치는 없는 상태

plotResult(x, pred2)    # 분류가 제대로 됨. 모양따라 군집 형성
# KMeans는 k개의 군집의 갯수를 맞추고, DBSCAN은 밀도에 의해 형태를 맞춘다.